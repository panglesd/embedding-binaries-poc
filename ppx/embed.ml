...
let expand ~ctxt filepath =
  ...
  let cwd = Expansion_context.Extension.code_path ctxt |> Code_path.file_path |> Filename.dirname in
  let filepath =
    if Filename.is_relative filepath then
      Filename.concat cwd filepath
    else filepath
  in
  try
